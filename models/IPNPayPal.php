		...
		if (curl_errno ( $ch ) != 0) // cURL error
{
	...
	public function processPayment($_req, $req) {
		...
		$ordenCompra = PayOrdenesCompras::find()->where(['txt_order_number'=>$item_number, 'b_pagado'=>0])->one();
	...
	public function existeTransaccion($txn_id) {
        ...
        $existeTransaccion = PayPaymentsRecibed::find()->where(['txt_transaccion'=>$txn_id])->one();
